...
-- Create policies for finances table (basic - can be refined later)  
CREATE POLICY "Authenticated users can view finances" ON finances FOR SELECT TO authenticated;
CREATE POLICY "Authenticated users can create finances" ON finances FOR INSERT TO authenticated WITH CHECK (true);
CREATE POLICY "Users can update own finances" ON finances FOR UPDATE TO authenticated USING (created_by::text = auth.uid()::text);
        `
      }, { status: 200 })
    }
